...
export default function Navbar({ scrolled, onNavigateHome, isProductPage }) {
  ...
  return (
    <>
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 sm:px-6 lg:px-10">
            ...
            <div className="flex flex-col">
              <span
                className={`font-serif text-xl sm:text-2xl font-semibold tracking-tight transition-colors ${
                  activeHeader ? "text-foreground" : "text-white"
                }`}
              >
                Solarium
              </span>
              <span
                className={`text-[9px] sm:text-[10px] uppercase tracking-[0.28em] font-medium -mt-1 transition-colors ${
                  activeHeader ? "text-accent" : "text-white/80"
                }`}
              >
                Shutters & Blinds
              </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <a
              ...
              className={`rounded-full border px-3.5 py-1.5 sm:px-5 sm:py-2.5 text-[11px] sm:text-xs font-medium uppercase tracking-[0.16em] sm:tracking-[0.18em] transition-all ${
                activeHeader
      ...
      <div
        className={`fixed inset-x-0 top-[59px] sm:top-[63px] bottom-0 z-40 bg-cream/98 backdrop-blur-xl border-b border-border transition-all duration-300 ease-in-out lg:hidden flex flex-col justify-between px-6 py-8 overflow-y-auto ${
          mobileOpen
