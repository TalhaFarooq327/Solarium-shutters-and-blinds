    ...
    <section className="border-y border-border bg-cream/70 backdrop-blur-xs overflow-hidden">
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-y-10 px-6 py-14 sm:grid-cols-3 md:grid-cols-5 lg:px-10">
        {trust.map(({ icon: Icon, stat, label }, idx) => (
          ...
          >
            <div className="grid h-12 w-12 place-items-center rounded-full bg-white/80 shadow-2xs ring-1 ring-border/80 transition-all duration-300 group-hover:scale-110 group-hover:bg-accent/15 group-hover:ring-accent/40">
              <Icon className="h-5 w-5 text-accent transition-transform duration-300 group-hover:scale-110" strokeWidth={1.5} />
            </div>
            <div className="mt-3.5 font-serif text-2xl font-semibold text-charcoal">{stat}</div>
